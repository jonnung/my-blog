...
레이어는 도커 이미지가 빌드될 때 `Dockerfile`에 정의된 명령(Instructions)을 순서대로 실행하면서 만들어진다. 이 레이어들은 각각 독립적으로 저장되며 읽기 전용이기 때문에 임의로 수정할 수 없다.   
...
## 도커 이미지 레이어에 대해
도커 이미지 레이어가 중요한 이유는 이미지를 다시 빌드할 때마다 이미 생성된 레이어가 캐시 되어 재사용 되기 때문에 빌드 시간을 단축할 수 있다.   
...
그 이유는 다른 리눅스 배포판고 비교해 사이즈가 훨씬 작기 때문이다. Alpine 리눅스는 용량이 적은 비결은 C 라이브러리로 `musl`을 채택했고, 다양한 유닉스 도구들을 탑재한 `busybox`를 기반으로 하고 있기 때문이다. 

그러나 만약 **Python 애플리케이션 환경**에서 베이스 이미지를 선택할 때는 Alpine 리눅스를 사용하는 것은 추천하지 않는다. 그 이유에 대한 설명은  [Using Alpine can make Python Docker builds 50× slower](https://pythonspeed.com/articles/alpine-docker-python/) 글을 읽어보는 것을 추천한다.  

{{< image src="/images/docker_python.png" position="center" style="width: 70%">}}

요약 하자면 PyPI에 올려진 파이썬 라이브러리들은 보통 `Wheel` 포맷을 사용하는 데 Alpine 리눅스는 `Wheel` 포맷을 지원하지 않기 때문에 파이썬 소스 코드(.tar.gz)를 내려받아 직접 컴파일해야 하는 경우가 있다.    
`Wheel` 포맷을 지원할 수 없는 원인은  `musl` 라이브러리가 GNU C 라이브러리(`glibc`)로 컴파일된 Wheel 바이너리를 지원하지 않기 때문이다. 그래서 PyPI를 통해  `.whl`로 된 패키지가 아닌 소스 코드(`.tar.gz`)를 다운로드받게 되는 것이다. 

Python `Wheel` 포맷은 설치 없이 Python에서 바로 실행할 수 있다는 장점이 있다. 그 비결은 `Wheel` 포맷 자체가 `zip` 압축에서 확장자만 바꾼 것이며, Python은 Java의 `.jar`처럼 `zip` 파일을 바로 실행할 수 있기 때문이다.   

따라서 Alpine 리눅스 이미지를 사용한다면 모든 Python 패키지에서 C 코드를 컴파일 해야 하므로 도커 이미지를 빌드하는 시간이 상당히 많이 소모된다. 
...
그리고 Alpine 리눅스 이미지를 베이스 이미지로 사용한 Python 이미지는 용량도 크고, 몇 가지 버그들에 대한 취약점도 있다고 한다.   

Python 애플리케이션의 도커 이미지 선택의 결론은 **Debian Buster**를 기반으로 한 공식 Python 이미지 `python:3.8-buster` 또는 `3.8-slim-buster`를 사용하는 것을 추천한다.   
...
### (2) 이미지 레이어 개수를 줄이자
과거 도커 버전에서는 이미지 레이어 개수가 성능에 영향을 줬다고 한다. 하지만 이제는 그렇지 않다고 한다.

도커 레이어를 줄이는 것도 최적화 측면에서 도움이 된다고 볼 수 있다.   
레이어는 `RUN`, `ADD`, `COPY` 명령문에서만 생성되기 때문에 아래와 같이 여러 개로 분리된 명령을 **체이닝(chaining)** 으로 엮는 것도 좋은 방법이다.   
...
`apt-get install` 로 설치할 패키지들의 순서를 알파벳 순서로 정렬하면 가독성도 높고, 중복 설치를 방지할 수 있는 효과도 있다.
...
하지만 두 번째 `COPY`는 빌드할 때마다 바뀔 수 있기 때문에 캐시가 자주 초기화될 것이고, 다음 실행되는 `RUN`명령어를 통해 의존성 패키지 설치가 매번 실행될 가능성이 높다. 

따라서 **애플리케이션 코드 복사 명령은 자주 변경되지 않는 명령문 다음에 오는 것이 이미지 빌드 시간을 단축하는 데 유리하다.**   

<br/>

### (4) 애플리케이션 패키지 매니저의 Lock 파일 시스템을 활용
...
이 도구들을 이용해 생성된 **Lock 파일** (예: `Pipfile`)을 기반으로 패키지가 설치될 수 있도록 한다면 위 (3) 단락에서 얻은 장점을 동일하게 얻을 수 있다. 
...
이미지 빌드 과정에서 생성된 것 중 사용되지 않거나 불필요한 모든 것들을 무시하고, 실제 컨테이너 실행에 필요한 부분만 가져와서 새로운 BASE 이미지로부터 다시 새 이미지를 생성할 수 있다.    

아래 샘플을 통해 살펴보자.   
`Pipfile`에는 `uwsgi` 패키지 의존성을 명시하고 있고, 이 패키지를 설치하기 위해서는 `gcc`가 필요하기 때문에 `apt-get install gcc`를 실행해야 한다.    

여기서 예상할 수 있는 것은 `gcc`는 `uwsgi`설치에만 관여하고 실제 애플리케이션 실행 환경에서는 필요하지 않기 때문에 멀티-스테이지 빌드를 사용한다면 이 부분이 구분점이 될 수 있을 것이다.    
...
마지막 부분에 있는 `COPY --from=builder`를 통해 전 단계 스테이지 빌드에서 생성된 특정 결과물만 새로운 BASE 이미지로 복사해서 이미지를 생성했다.   
