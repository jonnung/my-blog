...
패키지를 import할 때 import한 패키지 경로의 마지막 부분을 네임스페이스처럼 그 패키지 안에 있는 메서드를 호출할 때 사용된다.  
...
```
<br/>
함수의 반환값 Type에 미리 변수명을 선언하고, 함수 안에서 바로 사용할 수 있다.  
그리고 그냥  `return` 하더라도 함수의 반환값으로 선언한 변수가 자동으로 반환된다.  
...
### 변수
변수는 `var`키워드를 사용해서 선언하고 변수명 다음에 Type을 명시한다.  
...
<br/>
오직 함수 안에서만 `:=`로 변수를 선언하면 암시적으로 Type을 유추한다.  
...
각 타입의 Zero Value는 다음과 같다.  

- 숫자 Type: 0
- Boolean Type: false
- String Type: ""(빈 문자열)  

타입 변환은 기존 변수를 Type 이름으로 감싸는 형태로 변환할 수 있다.  
...
'반복 조건 표현식'이 `false`가 되면 반복은 종료된다.   
...
<br/>
마지막으로 for문의 각 구성 요소는 필수가 아니다.   
오직 '반복 조건 표현식'만 사용한다면, 다른 언어에도 존재하는 `while`이 되고, 전부 생략할 경우 무한 루프를 의미한다.  
...
그리고 `if`문 조건식에도 짧은 변수를 선언할 수 있고, `for`문과 마찬가지로 이 짧은 변수도 `if`문안에서만 사용할 수 있다.    
...
위 방법이 정석적이라면 좀 더 간편하게 Map 리터럴을 사용할 수 있다.  
